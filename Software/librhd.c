...
/************************** Version control information ***************************/
 #define LIBRHD_VERSION   "2.2"
 #define LIBRHD_REVISION  "$Rev: 1963 $:"
...
ssize_t secureWrite(int fd, const char *buf, ssize_t txLen);
ssize_t secureRead(int fd, char *buf, ssize_t txLen);
 ...
 */
char rhdConnect(char rw, char *hostname, int port) {
  ...
  //Run the package parser to recieve acknowledgement and symboltables
	...
	for(i = 0; i < rTableLen; i++) rSymTable[i].updated = 0;
	for(i = 0; i < wTableLen; i++) wSymTable[i].updated = 0;
...
int recieveSymboltable(char dir) {
  ...
  if(secureRead(rhdSocket, (char*)&tempTableSize, sizeof(int32_t)) <= 0) 
    return -1;
  ...
  for(i = 0; (i < tempTableSize); i++) {
    if(secureRead(rhdSocket, (char*)&tempElement.length,sizeof(tempElement.length)) <= 0) 
      return -1;
    ...
    // Count the size of the datapool (timestamp + number of values
    // tempDataSize += sizeof(struct timeval)/sizeof(int) + tempElement.length;
    tempDataSize += 2 + tempElement.length;
  }
  #if DEBUG
  printf("DEBUG: %c sym table size=%d\r\n", dir, tempTableSize);
  #endif
  ...
  for(i = 0, tmpPtr = tempDatapool; (i < tempTableSize); i ++) {
    tempTable[i].timestamp.tv_sec = *tmpPtr++;
    tempTable[i].timestamp.tv_usec = *tmpPtr++;
...
int recieveDatapackage (char dir) {
  ...
  if (dir == 'w') {
    tempTable = wSymTable;
    tempTableLength = wTableLen;
    //tempDatapool = wData;
  } else if (dir == 'r') {
    tempTable = rSymTable;
    tempTableLength = rTableLen;
    //tempDatapool = rData;
  } else {
	  return -1;
  }
  #if DEBUG
  printf("DEBUG: %c sym table (length=%d)\r\n", dir, tempTable->length);
  #endif
  
  //Read size of sync package from server
  if (secureRead(rhdSocket, (char*)&pkgSize, sizeof(int32_t)) <= 0) {
      return -1;
  }
  ...
  for(pkg = pkgSize; pkg > 0; pkg--) {
      //Read variable id
      if (secureRead(rhdSocket, (char*)ioData, sizeof(int32_t)) <= 0) {
        return -1;
      }
      ioData[0] = ntohl(ioData[0]); //Convert to host order
//       if (ioData[0] >= tempTable->length)
      if (ioData[0] >= tempTableLength)
      { // error
        printf("                                      Received an %c package, error (index %d >= table length %d!)\r\n", dir, ioData[0], pkgSize);
        return -1;
      }
      //Read variable data
      if (secureRead(rhdSocket, (char*)(ioData+1), 2 * sizeof(int32_t) + (tempTable[ioData[0]].length * sizeof(int32_t))) <= 0) {
        return -1;
      }
...
int transmitWritepackage (void) {
	...
	// iodata is pointing to next item in transmit buffer (variable counter)
	ioData = (int32_t *) ioBufPtr;
    // ioPtr points to transmit buffer for first value
	...
	for(i = 0; i < wTableLen; i++) {
		if (wSymTable[i].updated >= 1) {
			*ioPtr = i; //Save index
			ioPtr++;    //Move pointer
			//Copy timestamp
			//memcpy(ioPtr, wSymTable[i].timestamp, sizeof(struct timeval));
			//ioPtr += sizeof(struct timeval) / sizeof(int32_t);
			*ioPtr = (int32_t) wSymTable[i].timestamp.tv_sec;
         		ioPtr++;
         		*ioPtr = (int32_t) wSymTable[i].timestamp.tv_usec;
         		ioPtr++;
			
			//Copy data
			memcpy(ioPtr, wSymTable[i].data, wSymTable[i].length * sizeof(int32_t));
			ioPtr += wSymTable[i].length;
			ioData[0]++;
			wSymTable[i].updated = 0; //Remove updated mark
		}
	}
...
int transmitTxBuffer(void) {
   ...
   if (secureWrite(rhdSocket, (char*) ioBuf,(ssize_t)(ioBufPtr - ioBuf)) <= 0) {
 ...
 */
int writeValue(int id, int index, int value) {
...
//	printf("Writing value: %s\n",wSymTable[id].name);
  ...
  wSymTable[id].data[index] = value;
  wSymTable[id].updated = 1;
  gettimeofday(&wSymTable[id].timestamp,NULL);
 ...
 * Returns the if the variable is successfully updated
 */
int writeValueNamed(char *varName, int index, int value) {

    int i;
    for (i = 0; i < wTableLen; i++) {
      if (!strcmp(varName,wSymTable[i].name)) {
        return writeValue(i,index,value);
      }
    }
 ...
 * Returns the if the variable is successfully updated
 */
int writeArray(int id, int length, int* array) {
  
//Check bounds of update
  if ((id < 0) || (id >= wTableLen) || (length < 0) || 
      (length > wSymTable[id].length)) {
    return -1;
  }
  memcpy(wSymTable[id].data, array, length * sizeof(int32_t));
  wSymTable[id].updated = 1;
  gettimeofday(&wSymTable[id].timestamp,NULL);
...
int writeArrayNamed(char *arrayName, int length, int* array) {
    ...
    int i;
    for (i = 0; i < wTableLen; i++) {
      if (!strcmp(arrayName,wSymTable[i].name)) {
        return writeArray(i,length,array);
      }
    }
  //Return error-value if no name found
  return -1;  
 ...
 * \returns int value
 */
int readValue(int id, int index) {
  ...
  return rSymTable[id].data[index];
}
 ...
 * \returns int array
 */
  int readValueNamed(char *valName, int index) {
  
    int i;
    for (i = 0; i < rTableLen; i++) {
      if (!strcmp(valName,rSymTable[i].name)) {
        return readValue(i,index);
      }
    }
  //Return error-value if no name found
  return 0;
 ...
 * \returns int* array
 */
int*  readArray(int id) {
  //Check bounds of update
  if ((id < 0) || (id >= rTableLen)) {
    return NULL;
  }
  return (int32_t*)rSymTable[id].data;
}
 ...
 * \returns int* array
 */
  int32_t * readArrayNamed(char *arrayName) {
  
    int i;
    for (i = 0; i < rTableLen; i++) {
      if (!strcmp(arrayName,rSymTable[i].name)) {
        return readArray(i);
      }
    }
    //Return error-value if no name found
...
int isRead(int id) {
  ...
  else return rSymTable[id].updated;
 ...
 * \returns int updated
 */
int isReadNamed(char *varName) {

  int i;
  for (i = 0; i < rTableLen; i++) {
    if (!strcmp(varName,rSymTable[i].name)) {
      return isRead(i);
    }
  }
  //Return error-value if no name found
