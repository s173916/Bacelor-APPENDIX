/** \mainpage
 * The MRC program is developed for educational
 ...
 * \brief Main file of the mrc program. 
 * 
...
double gyro1 = 0, gyro1off = 0, gyro1gain=1.0, gyro2 = 0, gyrotemp1 = 0, gyrotemp2 = 0;
// ESM - added by Jørgen Eriksen
...
#define PARAMETERLISTSIZE 20
double notconnected=0;
double defaultobstaclefront=50,defaultobstacleback=50;
...
addpar(&odoconnectparlist,"steeringangle",ADMIN_inp,ADMIN_double,&odoconnector.steeringangle);
  ...
  addpar(&motionconnectparlist,"yaw",ADMIN_inp,ADMIN_double,&motionconnector.yaw);
  ...
  motionconnector.yaw=&notconnected;
...
struct {
   posetype data[MAXLINESENSORSIZE];
   double lsold[MAXLINESENSORSIZE];
...
typedef struct {
   double *pdrivendist, *pcmdtime,
       ...
       *podox, *podoy, *podoth,*pthw, *pododist, *pododistleft, *pododistright,
...
void update_internvar(internvartype * p, sm_type * sm)
{
   ...
   *p->preadflags = 0;//robot->read_flags;
   ...
   *p->l8=laserpar[8];
   *p->l9=laserpar[9];
   *p->pclock=0.01;//robot->ts;
   *p->ptick=0;//robot->tick;
...
//    uint32_t smrdtime;
   ...
   unsigned char sig;
   double dummyinp=0;
   double *xyzout4,*xyzout1,*xyzout2;
...
kalmanodo.process_noise_std_dist); */
     // robot = &smrdummy;
      if (rhdConnect('w',robothost,robot_port)!='w'){
	 ...
	 exit(EXIT_FAILURE); 
      ...
      xyzout4=putinternvar("xyzout4",1);
      xyzout1=putinternvar("xyzout1",1);
      xyzout2=putinternvar("xyzout2",1);
      ...
      yawrateref=getoutputref("yawref",outputtable);
	 ...
	 fgets(ansbuf,80,stdin);
         ...
         case 2:{
	       ...
	       int npar=0;
		     ...
		     printf("You entered  ycw = %lf  yccw = %lf  L = %lf OK? (y,n)\n",
		            ycw,yccw,L);
			   fgets(ansbuf,80,stdin);
		  ...
		  while (npar < 5){
		     ...
		     printf("You entered xcw = %lf ycw = %lf xccw = %lf yccw = %lf  L = %lf OK? (y,n)\n",
		            xcw,ycw,xccw,yccw,L);
			   fgets(ansbuf,80,stdin);
                     if (ansbuf[0] != 'y' && ansbuf[0] != 'Y') npar=0;
		    }      
...
//       exit(gps.client.error);
   ...
   switch (robotinfo.type){
	...
	// odo.dth =*rotpos->data/40000.;
      ...
      //smrdtime = robot->ts;                    naa 13-1-2015
...
//         fprintf(stderr,"Using gps measurement\n");
	...
	//if ((count & 0x7f)==0)
        ...
        while ( (xml_in_fd(xmldata,camsrv.sockfd) >0))
       	...
       	if (siminterface.simulating){
	   simtime=isimtime->data[0]+(double)isimtime->data[1]/1000000.;
	}
...
//printf("------ ER HER2----------------------\n\n\n\n\n");
	...
	motcon.pose3d.yaw=*motionconnector.yaw;
        motcon.tick = count;
...
//printf("------ ER HER14--ikke return kalman--------------------\n\n\n\n\n");
          }
...
//printf("------ ER HER19 ----------------------\n\n\n\n\n");
      ...
      //**************************************************
      *xyzout4=motcon.xyzout4;
       *xyzout1=motcon.xyzout1;
       *xyzout2=motcon.xyzout2;
      switch (robotinfo.type){
      case VELOMEGA:
        if (cmdtransvel != NULL)
        {
...
//printf("------ ER HER20 ----------------------\n\n\n\n\n");
      ...
      case HEXACOPTER:
          if (thrustref) {thrustref->inputVar[0]=motcon.xyzout3;rhdwrite(thrustref,0);}	
	  if (pitchref) {pitchref->inputVar[0]=motcon.xyzout1;rhdwrite(pitchref,0);}
	  if (rollref) {rollref->inputVar[0]=motcon.xyzout2;rhdwrite(rollref,0); }
          if (yawrateref) {yawrateref->inputVar[0]=motcon.xyzout4;rhdwrite(yawrateref,0); }	  	  	  
      break;
